...
"""Deterministic Stage 4 MPS multi-GPU validation job."""
...
def build_parser():
    ...
    parser.add_argument("--davidson-sites", choices=("one", "two"), default="two")
    return parser


def _site_method(value):
    return {"one": "1site", "two": "2site"}[value]


def _state_arrays(state):
    tensors = [_host(matrix.array) for matrix in state]
    qn_arrays = [np.asarray(qn) for qn in state.qn]
...
def _state_hash(state):
    ...
    digest.update(str(state.qnidx).encode("ascii"))
    digest.update(str(bool(state.to_right)).encode("ascii"))
...
def _mps_model(args):
    ...
    if args.model_size < 4:
        raise ValueError("MPS model-size must be at least 4")
    np.random.seed(args.seed)
    model = Model(
        [BasisHalfSpin(index) for index in range(args.model_size)],
        heisenberg_ops(args.model_size),
    )
    return Mps.random(model, 0, args.bond_dimension, 1), Mpo(model)


def _run_tdvp(args, initial, mpo, execution):
    ...
    def setup():
        ...
        if args.tdvp_sites == "two":
            distributed.compress_config = CompressConfig(
                CompressCriteria.fixed, max_bonddim=args.bond_dimension
            )
        selected = backend.current
        reference_receive = selected.array_namespace.empty(
            2 ** args.model_size, dtype=np.complex128
        )
        return method, distributed, selected, reference_receive

    method, distributed, selected, reference_receive = _synchronized_action(
        execution, setup, "TDVP workflow setup"
    )
    ...
    def reference_operation():
        ...
        if args.tdvp_sites == "two":
            reference.compress_config = CompressConfig(
                CompressCriteria.fixed, max_bonddim=args.bond_dimension
            )
        reference = reference.evolve(mpo, args.tau, normalize=False)
        return selected.array_namespace.ascontiguousarray(
            selected.asarray(reference.todense(), dtype=reference_receive.dtype)
        )

    reference_dense = _run_root_reference(
        execution, reference_operation, reference_receive
    )
    _synchronized_action(
        execution,
        lambda: setattr(distributed.evolve_config, "distributed_execution", execution),
        "TDVP distributed config",
    )
    distributed = distributed.evolve(mpo, args.tau, normalize=False)

    def compare_results():
        distributed_dense = _host(distributed.todense())
        reference_host = _host(reference_dense)
        return {
            "max_abs_error": float(
                np.max(np.abs(distributed_dense - reference_host))
            ),
            "tdvp_norm_error": float(
                abs(
                    np.linalg.norm(distributed_dense)
                    - np.linalg.norm(reference_host)
                )
            ),
        }

    return distributed, _synchronized_action(
        execution, compare_results, "TDVP result allocation"
    )


def _run_davidson(args, initial, mpo, execution):
    ...
    def setup():
        ...
        distributed = initial.copy()
        distributed.optimize_config.procedure = procedure
        distributed.optimize_config.method = _site_method(args.davidson_sites)
    ...
    procedure, distributed, selected, reference_receive = _synchronized_action(
        execution, setup, "Davidson workflow setup"
    )

    def reference_operation():
        reference = initial.copy()
        reference.optimize_config.procedure = procedure
        reference.optimize_config.method = _site_method(args.davidson_sites)
        reference.optimize_config.algo = "davidson"
        reference.optimize_config.nroots = 1
        reference_energies, reference = optimize_mps(reference, mpo)
        return selected.asarray(
            [min(reference_energies), reference.mp_norm], dtype=np.float64
        )
    ...
    _synchronized_action(
        ...
        "Davidson distributed config",
    )
    distributed_energies, distributed = optimize_mps(distributed, mpo)

    def summarize_result():
        energy = float(min(distributed_energies))
        expectation = float(np.real(distributed.expectation(mpo)))
        return {
            "energy_error": abs(energy - reference_energy),
            "residual_norm": abs(expectation - energy),
            "davidson_norm_error": abs(float(distributed.mp_norm) - 1.0),
            "davidson_reference_norm": reference_norm,
            "energy": energy,
        }

    return distributed, _synchronized_action(
        execution, summarize_result, "Davidson result allocation"
    )
...
def _root_summary(args, runtime, output_dir, rank_record):
    ...
    logical_fallback_count = max(fallback_counts, default=0)
    ...
    return {
        ...
        "fallback": {"count": logical_fallback_count, "root_operation_count": root_operation_count, "synchronized": True},
        "ranks": records,
        "reasons": reasons,
    }
...
def main(argv=None):
    ...
    with cupy_distributed_runtime(
        ...
        initial, mpo = _synchronized_action(
            runtime, lambda: _mps_model(args), "model setup"
        )
        numerical = {}
        final_state = initial
        if args.mode in {"tdvp", "both"}:
            final_state, result = _run_tdvp(args, initial, mpo, execution)
            numerical.update(result)
        if args.mode in {"davidson", "both"}:
            final_state, result = _run_davidson(args, initial, mpo, execution)
            numerical.update(result)
        ...
        synchronize_mps_update(
            ...
            tensors,
            qn_arrays=qn_arrays,
            metadata=(final_state.qnidx, bool(final_state.to_right), args.mode),
        )
