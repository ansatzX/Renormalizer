...
MODE=${1:?usage: run_tree_multigpu_job.sh MODE OUTPUT_DIR [job options...]}
OUTPUT_DIR=${2:?usage: run_tree_multigpu_job.sh MODE OUTPUT_DIR [job options...]}
shift 2
...
for sample in range(2):
    ...
    applications = query(["--query-compute-apps=gpu_uuid,pid,used_gpu_memory", "--format=csv,noheader,nounits"])
...
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
    -m renormalizer.tn.distributed_job \
